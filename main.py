...
def generate_educational_report(data: ReportGenerateRequest, x_token: str = Header(..., alias="X-Token")):
    ...
    try:
        ...
        parsed_fields = parse_ai_response(ai_text)
...
def parse_ai_response(response_text: str) -> Dict[str, str]:
    """تحليل النص الذي يرجع من الذكاء الاصطناعي إلى حقول"""
    ...
    # إذا فشل التحليل، نستخدم النصوص الافتراضية
    if not any(parsed.values()):
        for key in parsed:
            parsed[key] = DEFAULT_REPORT_TEXTS.get(key, [""])[0]
